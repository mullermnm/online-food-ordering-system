...
<body>
<?php 
        if (isset($_POST['submit-location-text'])) {
            $location = $connect->real_escape_string($_POST['location']);
            $username = $_SESSION['username'];
            if (isset($_GET['date'])) {
            $date = $_GET['date'];
            $save_location = "UPDATE orders SET locations='$location' WHERE username= '$username' and order_date= '$date' ";                
            }
            if (isset($_GET['id'])) {
                $food_id = $_GET['id'];
                $save_location = "UPDATE orders SET locations='$location' WHERE username= '$username' and id= '$food_id' ";
            }
            if ($connect->query($save_location)) {
                if (isset($_GET['id'])) {
                    $delete = "DELETE FROM review_ask where food_id = '$food_id'";
        ...
        if (isset($_POST['submit-location-gps'])) {
            #here i will save the link of the google map of the person
            header('location: myOrders.php'); 
        }
   ?>
<nav class="navbar">    
    ...
    <div class="location-teller">
        <p>Online Food Ordering System / Location</p>
    </div>
    <div class="login-form col-md-8 col-sm-8">
        <form method="POST">
            <label for="location" class="form-control-static">Enter Your Location:</label>
            <textarea name="location" id="" cols="30" rows="10" class="form-control" style="resize: none;"></textarea>
            <button class="btn login-btn" type="submit" name="submit-location-text">Submit</button>
        </form>
        <div class="gps-taker">
            <form method="POST">
                <h3>Share your location using google maps</h3>
                <button name="submit-location-gps" type="submit" class="btn gps-btn">Get LOcation Using GPS</button>
            </form>
        </div>
    </div>
    <div class="register-adds col-md-4 col-sm-4">
        <h3>Enter Your Location using Google</h3>
        <img src="images/gps-location.png" alt="gps" class="img-rounded">
        <p>we can get your location using google maps or you can direct us to your place or where you want</p>
    </div>
