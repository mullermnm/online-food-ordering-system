...
<body>
    <?php 
        ...
        if (isset($_POST['register'])) {
            ...
            $phone_number = $connect->real_escape_string($_POST['phone']);
            $password_1 = $connect->real_escape_string($_POST['password_1']);
            $password_2 = $connect->real_escape_string($_POST['password_2']);
            if ($username == "" || $username == 'admin') {
                $input_err = "<div class='alert alert-danger'>Invalid Username</div>";
            }
            else if ($phone_number == "") {
                $input_err = "<div class='alert alert-danger'>Invalid Phone Number</div>";
            }
            else if ($password_1 == "" or $password_2 == "") {
                $input_err = "<div class='alert alert-danger'>Password Required</div>";
            }
            else if ($password_1 != $password_2) {
                $input_err = "<div class='alert alert-danger'>Password Does Not Match</div>";
            }
            else {
                $insert_user = "INSERT INTO users (username, phone, password) VALUES ('$username', '$phone_number', '$password_1')";
                if ($connect->query($insert_user)) {
...
<nav class="navbar">    
    ...
    <div class="location-teller">
        <p>Online Food Ordering System / sign up</p>
    </div>
    <div class="col-md-8 col-sm-8 login-form">
        <a href="index.php"><button class="back-home">&times;</button></a> 
        <h3 class="text-center" style="color: chocolate">Register Here!</h3>
        <form method="post">
            ...
            <label for="phone" class="form-control-static">Phone Number: </label>
            ...
            <label for="password_2" class="form-control-static">Confirm Password: </label>
    ...
    <div class="register-adds col-md-4 col-sm-4">
        ...
        <img src="images/register-security.jpg" alt="security" class="img-rounded">
